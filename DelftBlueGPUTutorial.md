...
Install Julia's CUDA package on visual node, since visual nodes have acces to both a GPU and the internet. When adding CUDA to Julia, we need to make sure that CUDA always uses the locally installed toolkit, this way we avoid CUDA from downloading artifacts from the internet. CUDA can be installed by running the following batch script:
...
# CUDA.set_runtime_version!(local_toolkit=true) ensures that julia will always use the locally installed CUDA version
...
After having been granted acces to the interactive node, install CUDA and cuDNN. 
