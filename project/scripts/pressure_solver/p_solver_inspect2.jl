...
function rescale_pressure!(sim::BiotSimulation, Δt_new::Real)
    Δt_old = sim.flow.Δt[end]
    @show Δt_old, Δt_new
    if Δt_old > 0 && Δt_new > 0
        sim.flow.p .*= (Δt_old / Δt_new)
    end
end
...
clims = (-2, 1)
sim.flow.u .= u
Thesis.custom_sim_step!(sim)
# WaterLily.CFL(sim.flow)
forces1 = get_forces(sim)
mean_p_1 = mean(sim.flow.p .* sim.flow.μ₀[:, :, 2])
@show mean_p_1
@show forces1

plt1 = WaterLily.flood(
                sim.flow.p .* sim.flow.μ₀[:, :, 2],
                # sim.flow.σ,
...
Δt_new = 0.005
# rescale_pressure!(sim, Δt_new)
push!(sim.flow.Δt, Δt_new)
Thesis.custom_sim_step!(sim)
WaterLily.CFL(sim.flow)
...
time = Δt_new*sim.U/sim.L

forces2 = get_forces(sim)
mean_p_2 = mean(sim.flow.p .* sim.flow.μ₀[:, :, 1])
@show mean_p_2
@show forces2
...
# WaterLily.plot_logger("simulation.log")
...
#         p = mean(sim.flow.p .* sim.flow.μ₀[:, :, 2])
#         @show p
#         # Record force
...
# rescale_pressure!(slow_sim, Δt_new)
# slow_pressure = []
# slow_times = []
# slow_forces = []
# next_save_slow = deepcopy(next_save)
# while sim_time(slow_sim) < t_extr
#     push!(slow_sim.flow.Δt, Δt_new)
#     Thesis.custom_sim_step!(slow_sim)

#     if sim_time(slow_sim) > next_save_slow
#         sim_info(slow_sim)

#         p = mean(slow_sim.flow.p .* slow_sim.flow.μ₀[:, :, 2])
#         @show p
#         # Record force
...
# # # @show sim_time(sim)
# # rescale_pressure!(sim, WaterLily.CFL(sim.flow))
