...
# node_path = "data/saved_models/inline_runs_hpc/base/NODE_Jun05-161406/node_params.jld2"
# AE_path = "data/saved_models/inline_runs_hpc/base/AE_Jun05-1610__E100_HW256x256_C4to2_nc5_nd1_z16_C8_lr0p0002_wd0p0009_bs16_NY_LL1_Tl0p0/checkpoint.jld2"

node_path = "data/saved_models/inline_runs_hpc/base/NODE_Jun05-162105/node_params.jld2"
AE_path ="data/saved_models/inline_runs_hpc/base/AE_Jun05-1617__E100_HW256x256_C4to2_nc5_nd1_z16_C8_lr0p0002_wd0p0009_bs16_NY_LL1_Tl0p0/checkpoint.jld2"
...
ae_args.full_data_path = base_path

@show ae_args.full_data_path
zs, ts, tspan, z0s = Thesis.get_latent_chunks(ae_bundle, normalizer, ae_args, downsample=ae_args.train_downsample)
...
_, predss = loss_multiple_shoot_multi(node, zs, z0s; p=node.p0, ts=ts, group_size=node_args.group_size, continuity_term=node_args.continuity_term)
plt = plot_multiple_shoot_multi(node, predss, zs; group_size=node_args.group_size, ts=ts, title_loss="")
savefig(plt, joinpath(node_dir, "multiple_shoot_multi.pdf"))
display(plt)
plt_rollout = Thesis.plot_node_rollout_multi(node, zs, z0s; p=node.p0, ts=ts, n_reconstruct=node_args.n_reconstruct)
display(plt_rollout)
savefig(plt_rollout, joinpath(node_dir, "rollout_multi.pdf"))
...
#              legendfontsize=7, framestyle=:box, gridalpha=0.20, gridlinewidth=0.5,
#              foreground_color_axis=:black, foreground_color_text=:black)
#     plot!(p, t_chunk, score .+ 0.06; color=:black, label="KNN score")
...
# p_knn0   = plot_knn_rollout(t0,  score0;  threshold=0.326)
# plot!(p_knn0, t, knn_curve(knn, z) .+ 0.1)

# println(0.99*maximum(score0))
