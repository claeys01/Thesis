...
# Load — node_args will be a JLD2.ReconstructedMutable
node, node_args_raw = load_node(NODE_path; verbose=true)

# Convert to current NodeArgs by copying matching fields
fields = fieldnames(NodeArgs)
kwargs = Dict{Symbol,Any}()
for f in fields
    if hasproperty(node_args_raw, f)
        kwargs[f] = getproperty(node_args_raw, f)
    end
end
node_args = NodeArgs(; kwargs...)

# Re-save with the current NodeArgs type
save_node(NODE_path, node, node_args)
@info "Re-saved NODE checkpoint with current NodeArgs definition"