...
function physical_ass(AE_path::String, NODE_path::String; saveplot=false)
    ...
    û_div = div_field(û; buff=1, avg=true)
    û_ε = kinetic_energy_dissipation(û; ν=sim.flow.ν, buff=1, avg=true)
    ...
    u_ε = kinetic_energy_dissipation(simdata.u[:, :, :, total_downsample]; ν=sim.flow.ν, buff=2, avg=true)
